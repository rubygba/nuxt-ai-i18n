...
program
    ...
    const targetLocales = locales.split(',').map((l) => l.trim()).filter(Boolean);
    console.log('\n🌐  nuxt-ai-i18n sync');
    console.log('─────────────────────────────────────────');
    ...
    for (const file of files) {
        ...
        for (const m of content.matchAll(/<AiI18nT[^>]+tKey=['"`]([^'"`]+)['"`]/g))
            keys.add(m[1]);
    }
    if (keys.size === 0) {
...
Keep translations concise and suitable for UI labels/buttons/messages.`,
                ...
                for (const key of batch) {
                    try {
                        ...
                        console.error(`    ✗ Failed to translate "${key}":`, e2.message);
                    }
                }
            }
        }
